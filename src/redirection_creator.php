...
<body>
  <div class="container">
  <h1>
  Création de Redirections <?php echo "@$domain"; ?>
  </h1>
<?php
     ...
     while ($tab = fgetcsv($fic, 1024, ';')) {
         ...
         else
         {
           ...
           try {
              ...
              $newAccount = $conn->post('/email/domain/' . $domain . '/redirection', array(
                'domain' 	=> $domain,
                'from' 	=> $from,
                'to'		=> $to,
                'localCopy' => false
               ...
               echo "<div class='alert alert-success' role='alert'>Redirection : $from vers $to créé</div>";

            } catch ( Exception $ex ) {
              echo "<div class='alert alert-warning' role='alert'>Redirection : $from vers $to problème...  <pre>";
...
{ 
 ?>
  ...
  <form method="POST" action="redirection_creator.php" enctype="multipart/form-data">
    <input type="hidden" name="envoi" value="1" />
    <div class="form-group">
      <label for="fichier">Choisissez le fichier contenant les redirections à créer</label>
<!--       <input type="file" id="fichier"> -->
      ...
      <p class="help-block">csv deux colonnes : adresse à rediriger et adresse destinataire</p>
    </div>
